...
- In the CLI, an Add-on is an npm plugin package (e.g. `vura-dataverse`, published from the same package as the VS Code extension) that `vura-runner` loads by name — declared by the notebook itself (`requiredPlugins`) or globally (`vura.plugins` config) — and registers the same way.
