...
The core extension acts as the orchestrator. When a notebook cell runs, it spawns an isolated sidecar process (Python or Node.js) communicating over local stdio (stdin/stdout) using JSON-line control messages. This sidecar uses our `@vura-data-os/vura-io` / `vura.io` library to save outputs natively into the workspace storage via Parquet/Arrow files. When the next cell runs—even in a different language—it queries that same dataset using DuckDB.
