...
install-core-extension: build-core-extension
	bash ./scripts/package-extension.sh core/core-extension -o dist/vura-studio-lite.vsix
	code --install-extension dist/vura-studio-lite.vsix
...
# Package every connector Add-on to dist/*.vsix without installing into local
# VS Code — same build as install-vura-* above, minus the final `code
# --install-extension` step. For producing Marketplace-upload artifacts.
#
# Each connector is built and immediately packaged before moving to the next
# — NOT built as a batch of prerequisites first. install-local-deps.sh (run
# by every build-vura-* target) does a root-level `npm install`, and
# vura-dataverse/vura-sharepoint are npm workspace members, so a later
# connector's root install resets an earlier workspace-member connector's
# staged node_modules back to the thin hoisted version before it gets
# packaged. Interleaving build+package per connector (via $(MAKE) so each
# build-vura-* target's own prerequisites still run) avoids that.
package-connectors-vsix:
	$(MAKE) build-vura-dataverse
	bash ./scripts/package-extension.sh connectors/vura-dataverse -o dist/vura-dataverse.vsix
	$(MAKE) build-vura-sharepoint
	bash ./scripts/package-extension.sh connectors/vura-sharepoint -o dist/vura-sharepoint.vsix
	$(MAKE) build-vura-onedrive
	bash ./scripts/package-extension.sh connectors/vura-onedrive -o dist/vura-onedrive.vsix
	$(MAKE) build-vura-googledrive
	bash ./scripts/package-extension.sh connectors/vura-googledrive -o dist/vura-googledrive.vsix
	$(MAKE) build-vura-s3
	bash ./scripts/package-extension.sh connectors/vura-s3 -o dist/vura-s3.vsix
	$(MAKE) build-vura-local
	bash ./scripts/package-extension.sh connectors/vura-local -o dist/vura-local.vsix
...
build-mac:
	bash ./scripts/install-local-deps.sh core/core-extension
	bash ./scripts/package-extension.sh core/core-extension --target darwin-x64 -o dist/vura-studio-lite-darwin-x64.vsix
	bash ./scripts/package-extension.sh core/core-extension --target darwin-arm64 -o dist/vura-studio-lite-darwin-arm64.vsix

build-linux:
	bash ./scripts/install-local-deps.sh core/core-extension
	bash ./scripts/package-extension.sh core/core-extension --target linux-x64 -o dist/vura-studio-lite-linux-x64.vsix
	bash ./scripts/package-extension.sh core/core-extension --target linux-arm64 -o dist/vura-studio-lite-linux-arm64.vsix

build-windows:
	bash ./scripts/install-local-deps.sh core/core-extension
	bash ./scripts/package-extension.sh core/core-extension --target win32-x64 -o dist/vura-studio-lite-win32-x64.vsix
	# No GitHub-hosted (or typical dev) win32-arm64 machine exists to resolve
	# the matching @duckdb/node-bindings-win32-arm64 optional dependency, so
	# TARGET_PLATFORM/TARGET_ARCH tell copy-duckdb-vendor.js (invoked inside
	# package-extension.sh) to substitute the pre-vendored binary at
	# vendor-packages/duckdb/win32-arm64/duckdb.node instead of whatever this
	# machine's own npm install resolved.
	TARGET_PLATFORM=win32 TARGET_ARCH=arm64 bash ./scripts/package-extension.sh core/core-extension --target win32-arm64 -o dist/vura-studio-lite-win32-arm64.vsix
