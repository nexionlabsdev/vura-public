...
- **[`packages/vura-dataverse`](packages/vura-dataverse/README.md)** — a single Add-on package implementing the Dynamics 365 / OData `$batch` sync engine, published to both npm (as a `vura-runner` CLI plugin) and the VS Code Marketplace (as an extension) from the same compiled output.
...
- **[`packages/vura-io-py`](packages/vura-io-py/README.md)** — the Python counterpart of `vura-io`'s state/metrics/JSON-shredding library, used by `vura-runner`'s Python sidecar to run `python` notebook cells. Published to PyPI as [`vura-io`](https://pypi.org/project/vura-io/).
