...
import { getGraphToken, graphContentUrl, graphFetch, SharePointProfileConfig } from './graphDriveClient';

export interface SharePointImportArgs {
    connection: string;
    path: string;
    format: string;
    target: string;
...
export interface SharePointExportArgs {
    connection: string;
    source: string;
    format: string;
    path: string;
...
async function resolveProfile(env: IVuraEnvironment, connectionId: string): Promise<{ profile: ConnectionProfile<SharePointProfileConfig>; secret: string | undefined }> {
    const profile = await env.getConnectionProfile(connectionId) as ConnectionProfile<SharePointProfileConfig> | undefined;
    if (!profile) {
...
export function parseImportArgs(commandLine: string): SharePointImportArgs {
    const tokens = tokenize(commandLine, '!sharepoint.import');
    const args: any = { format: 'csv' };
    readFlags(tokens, args);
    if (!args.connection) throw new Error('Missing required argument: --connection <id>');
    if (!args.path) throw new Error('Missing required argument: --path <document_library_file_path>');
    if (!args.target) throw new Error('Missing required argument: --target <table_name>');
    return args;
}

export function parseExportArgs(commandLine: string): SharePointExportArgs {
    const tokens = tokenize(commandLine, '!sharepoint.export');
    const args: any = { format: 'csv' };
    readFlags(tokens, args);
    if (!args.connection) throw new Error('Missing required argument: --connection <id>');
...
export async function handleSharePointImport(
    cell: FlownbCell,
    logger: ICellLogger,
    ...
    await logger.logText(`Downloading "${args.path}" from SharePoint site "${profile.config.siteUrl}"...`);
...
export async function handleSharePointExport(
    cell: FlownbCell,
    logger: ICellLogger,
    env: IVuraEnvironment,
    commandLine: string
    ...
    await logger.logText(`Exported ${records.length} record(s) from "${args.source}" to SharePoint path "${args.path}".`);
}
