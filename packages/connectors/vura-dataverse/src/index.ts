...
import { handleSyncDataverse, getToken } from './syncDataverseHandler';
import { describeDataverseSchema, DEFAULT_DATAVERSE_ENTITIES } from './schemaIntrospection';
...
export class DataverseProvider extends BaseAdapter implements IVuraProvider, IConnectionAdapter, IUIActionProvider {
    ...
    async describeSchema(connectionId: string): Promise<NormalizedSchema> {
        const connProfile = await this.env.getConnectionProfile(connectionId);
        let orgUrl: string;
        ...
        if (connProfile && connProfile.kind === 'dataverse') {
            ...
            const legacyProfile = await this.env.getProfile(connectionId);
            if (!legacyProfile) {
        ...
        const token = await getToken(authProfile, orgUrl, secret);
        return describeDataverseSchema(orgUrl, DEFAULT_DATAVERSE_ENTITIES, token);
    }
