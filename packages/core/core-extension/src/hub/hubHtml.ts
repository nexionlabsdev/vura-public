...
<div id="main">
  ...
  <div class="section" id="section-addons">
    <div class="row-flex" style="justify-content:space-between;align-items:center">
      <h2 style="margin:0">Add-on Marketplace</h2>
      <button class="secondary" id="refreshAddonsBtn">↻ Refresh</button>
    </div>
...
<script>
  ...
  let state = null;
  // isNewProfile starts false: renderConnectors() should default to an existing saved profile
  // when there is one, and only fall back to the blank "new connector" form when there are
  // none (or the user explicitly clicks "+ New Connector").
  let conn = { kinds: [], profiles: [], currentKind: '', currentProfileId: '', isNewProfile: false, fieldValues: {}, lastSelectedProfileId: '', activeProfileId: '' };
  ...
  document.getElementById('autoDetectBtn').addEventListener('click', () => vscode.postMessage({ type: 'autoDetectVenv' }));
  ...
  document.getElementById('refreshAddonsBtn').addEventListener('click', () => vscode.postMessage({ type: 'requestData' }));
