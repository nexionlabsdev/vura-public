...
async function main() {
    try {
        ...
        execSync(`bash "${path.join(rootDir, 'scripts/package-extension.sh')}" connectors/vura-sharepoint -o "${sharepointVsix}"`, { stdio: 'inherit' });
        execSync(`bash "${path.join(rootDir, 'scripts/package-extension.sh')}" connectors/vura-dataverse -o "${dataverseVsix}"`, { stdio: 'inherit' });
