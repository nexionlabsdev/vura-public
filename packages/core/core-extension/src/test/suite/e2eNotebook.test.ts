...
describe('Core Extension E2E Integration Suite', () => {
    ...
    before(async () => {
        ...
        const ext = vscode.extensions.getExtension('nexion-labs.vura-studio-lite');
        if (ext) {
    ...
    it('Executes Phase 5 magic commands through real notebook controller with behavioral and OS assertions', async () => {
        ...
        const extPath = vscode.extensions.getExtension('nexion-labs.vura-studio-lite')?.extensionPath || tempDir;
    ...
    it('Phase 8 export picker and Phase 9 UI commands are registered and executable via vscode.commands.executeCommand', async () => {
        ...
        assert.strictEqual(
            exportCell.document.getText().replace(/\r\n/g, '\n'),
        ...
        const origGetAllProfiles = ConnectionManager.getAllConnectionProfiles;
        ConnectionManager.getProfiles = () => [
            { id: 'profile-99', name: 'D365_Prod_Conn', authMode: 'ServicePrincipal', server: 'org.crm.dynamics.com' }
        ];
        ConnectionManager.getAllConnectionProfiles = (() => [
            { id: 'profile-99', name: 'D365_Prod_Conn', kind: 'dataverse', config: { authMode: 'ServicePrincipal', server: 'org.crm.dynamics.com' } }
        ]) as any;

        (vscode.window as any).showQuickPick = async (items: any[]) => items.find((i: any) => i.id === 'profile-99') || items[1] || items[0];
        ...
        try {
            ...
            ConnectionManager.getAllConnectionProfiles = origGetAllProfiles;
            (vscode.window as any).showQuickPick = origShowQuickPick;
        }
    ...
    it('Executes real installed connector package UI actions (vura-sharepoint & vura-dataverse) through extension activation and ProviderRegistry', async () => {
        ...
        const spExt = vscode.extensions.getExtension('nexion-labs.vura-sharepoint');
        assert.ok(spExt, 'vura-sharepoint extension should be installed in VS Code extension host');
        await spExt.activate();

        const dvExt = vscode.extensions.getExtension('nexion-labs.vura-dataverse');
        ...
        const spCellText = testCell.document.getText().replace(/\r\n/g, '\n');
        assert.strictEqual(
            spCellText,
        ...
        const dvCellText = testCell.document.getText().replace(/\r\n/g, '\n');
        assert.strictEqual(
            dvCellText,
