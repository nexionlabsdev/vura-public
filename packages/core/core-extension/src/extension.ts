...
import { ConnectionsConfigViewProvider } from './connectionsConfigViewProvider';
...
import { EnvironmentStatusService } from './hub/environmentStatusService';
import { exportCliCredentialBootstrap } from './exportCliCredentialBootstrap';
...
import { registerEnterpriseMode } from './enterprise/enterpriseMode';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    registerEnterpriseMode(context, outputChannel); // inert unless vura.enterprise.apiUrl is set

    ConnectionManager.initialize(context);

    const openHubCmd = safeRegisterCommand('vura.openEnvironmentHub', () => {
        HubPanel.createOrShow(context);
    });
    context.subscriptions.push(openHubCmd);

    const exportCliCredentialBootstrapCmd = safeRegisterCommand('vura.exportCliCredentialBootstrap', () => {
        exportCliCredentialBootstrap(context);
    });
    context.subscriptions.push(exportCliCredentialBootstrapCmd);

    const environmentStatus = new EnvironmentStatusService(context);
    environmentStatus.refresh();

    if (!context.globalState.get<boolean>('vura.hub.hasOpenedOnce')) {
        context.globalState.update('vura.hub.hasOpenedOnce', true);
        HubPanel.createOrShow(context);
    }

    // Register Webview Provider for the Connections sidebar (every connector kind, SQL included)
    const connectionsProvider = new ConnectionsConfigViewProvider(context.extensionUri, context);
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(ConnectionsConfigViewProvider.viewType, connectionsProvider)
    );
    ...
    const copyCellOutputCmd = safeRegisterCommand('vura-notebook.copyCellOutput', async (cell: vscode.NotebookCell) => {
        ...
        const tableName = cell.metadata?.tableName || `cell_${cell.index}`;
        if (context.storageUri) {
            await handleGridCopy(tableName, new VsCodeEnvironment(context, cell));
        } else {
    ...
    const cleanSessionCmd = safeRegisterCommand('vura-notebook.cleanSession', async (cellOrDoc?: any) => {
        ...
        const confirm = await vscode.window.showWarningMessage(
            'Purge this notebook session? This will terminate its running processes and permanently delete all session data (tables, connections, and cached state). This cannot be undone.',
            { modal: true },
            'Purge'
        );
        if (confirm !== 'Purge') {
        ...
        try {
            ...
            if (!env.notebookId || env.notebookId === 'global') {
                vscode.window.showErrorMessage('No active notebook session to purge.');
                return;
            }
            const { cleanNotebookSession } = require('@vura-data-os/vura-runner');
            await cleanNotebookSession(env);
            vscode.window.showInformationMessage(`Notebook session purged successfully.`);
        } catch (err: any) {
    ...
    return {
        ...
        registerProvider: async (id: string, provider: any) => {
            ...
            // Re-apply a previously-persisted disable (from the Environment Hub's Add-ons
            // section) — ProviderRegistry's enabled/disabled set is in-memory only and
            // starts fresh every activation, so the durable state lives in globalState.
            const wasEnabled = context.globalState.get<boolean>(`vura-addon-enabled-${id}`, true);
            if (!wasEnabled) {
                ProviderRegistry.getInstance().setProviderEnabled(id, false);
            }
        },
