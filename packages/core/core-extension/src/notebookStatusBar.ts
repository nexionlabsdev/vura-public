...
import { discoverWorkspaceVenvs, getVenvPythonBin, VenvCandidate } from '@vura-data-os/vura-runner';
...
export function registerNotebookStatusBarCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(safeRegisterCommand('vura-notebook.selectVenv', async () => {
        ...
        // Native OS "open folder" dialogs hide dotfiles (.venv, .env, …) by
        // default, with no VS Code API to override that — so the most common
        // venv location was effectively unselectable via "Select existing" /
        // "Create new"'s browse dialogs. Auto-detecting via discoverWorkspaceVenvs
        // (same helper the Environment Hub uses) sidesteps the OS dialog
        // entirely for the common case; browsing stays as a fallback for
        // anything not in a workspace root.
        const detected: (VenvCandidate & { workspaceFolder: string })[] = [];
        for (const folder of workspaceFolders) {
            const candidates = await discoverWorkspaceVenvs(folder.uri.fsPath);
            for (const c of candidates) {
                if (c.pythonBin) {
                    detected.push({ ...c, workspaceFolder: folder.name });
                }
            }
        }
        ...
        for (const d of detected) {
            if (d.folder === current) continue;
            items.push({
                label: `$(check) ${path.basename(d.folder)}`,
                description: d.folder,
                detail: `Auto-detected in "${d.workspaceFolder}"`
            });
        }

        items.push(
            {
                label: '$(folder-opened) Browse for existing venv folder…',
                description: 'For a venv outside the auto-detected locations above'
            },
            {
        ...
        const matchedDetected = detected.find(d => d.folder === pick.description);
        if (matchedDetected) {
            await context.workspaceState.update(VENV_KEY, matchedDetected.folder);
            updateVenvStatusBar(context);
            vscode.window.showInformationMessage(`Python venv set to: ${path.basename(matchedDetected.folder)}`);
            return;
        }

        if (pick.label.includes('Browse for existing')) {
            ...
            const pythonBin = getVenvPythonBin(selected);
            ...
            // Native "open folder" dialogs can be confusing to navigate (a
            // double-click to enter a folder vs. a single click + "Choose" to
            // select it look similar), so confirm the exact resolved path
            // before actually creating anything — cheap insurance against
            // creating a venv in the wrong directory.
            const confirm = await vscode.window.showWarningMessage(
                `Create venv at: ${venvPath}?`,
                { modal: true },
                'Create'
            );
