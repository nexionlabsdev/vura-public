...
class SidecarPool {
    ...
    /**
     * Host-facing alias for disposeNotebook — kills every sidecar worker for a
     * notebook on demand (e.g. a user clicking "Terminate" on a hung session in
     * the Environment Hub), not just on idle-reap.
     */
    async terminateSession(notebookId: string): Promise<void> {
        await this.disposeNotebook(notebookId);
    }

    /** Snapshot of every live pool entry, for a host UI to show active sessions. */
    listActiveSessions(): Array<{ key: string; notebookId: string; kind: string; workerCount: number; pids: number[]; anyBusy: boolean; idleMs: number }> {
        const now = Date.now();
        const sessions: Array<{ key: string; notebookId: string; kind: string; workerCount: number; pids: number[]; anyBusy: boolean; idleMs: number }> = [];
        for (const [key, workers] of this.pools.entries()) {
            if (workers.length === 0) continue;
            const [notebookId, kind] = key.split(':');
            sessions.push({
                key,
                notebookId: notebookId ?? key,
                kind: kind ?? 'unknown',
                workerCount: workers.length,
                pids: workers.map(w => w.proc.pid).filter((pid): pid is number => typeof pid === 'number'),
                anyBusy: workers.some(w => w.busy),
                idleMs: Math.min(...workers.map(w => now - w.lastUsed))
            });
        }
        return sessions;
    }
