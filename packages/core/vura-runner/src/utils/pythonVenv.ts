...
const VENV_SCAN_SKIP_DIRS = new Set(['node_modules', 'dist', 'out', '.git', '.vscode', '.vura', '__pycache__']);

async function scanDirForVenv(dir: string, candidates: VenvCandidate[]): Promise<void> {
 ...
 *
 * Checks the root itself, plus one level of immediate subdirectories (e.g.
 * `samples/.venv` in a repo whose workspace root is the repo top level) —
 * notebooks are often organized in a subfolder with its own venv rather than
 * one sitting at the literal workspace root.
 */
export async function discoverWorkspaceVenvs(workspaceRoot: string): Promise<VenvCandidate[]> {
    const candidates: VenvCandidate[] = [];

    await scanDirForVenv(workspaceRoot, candidates);

    try {
        const entries = await fs.readdir(workspaceRoot, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isDirectory()) continue;
            if (entry.name === '.venv' || entry.name === 'venv') continue; // already checked at root
            if (VENV_SCAN_SKIP_DIRS.has(entry.name)) continue;
            await scanDirForVenv(path.join(workspaceRoot, entry.name), candidates);
        }
    } catch {}
