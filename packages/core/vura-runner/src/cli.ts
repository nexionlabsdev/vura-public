...
credentialsCmd.command('add')
    ...
    .option('--port <port>', 'Server port (default 1433; Dataverse\'s TDS endpoint uses 5558)')
    ...
    .action(async (id, server, database, authMode, options) => {
        ...
        const profile: SqlProfile = {
            id, name: id, server, database, port: options.port ? parseInt(options.port, 10) : 1433, authMode: authMode as any,
...
credentialsCmd.command('update')
    ...
    .action(async (id, options) => {
        ...
        store[id].profile = {
            ...
            port: options.port !== undefined ? parseInt(options.port, 10) : current.port
        };
...
const connectionsCmd = program.command('connections').description('Manage generic connection profiles (SharePoint, S3, OneDrive, Google Drive, local folders, Dataverse, ...)');

connectionsCmd.command('list')
    .description('List stored connection profiles')
    .option('--kind <kind>', 'Filter by connector kind')
    .action(async (options) => {
        const env = new CliEnvironment(process.cwd());
        const profiles = await env.listConnectionProfiles(options.kind);
        if (profiles.length === 0) {
            console.log('No stored connection profiles found.');
        } else {
            console.log('Stored connection profiles:');
            profiles.forEach(p => {
                console.log(` - ${p.id} [${p.kind}] ${p.name}`);
            });
        }
    });

connectionsCmd.command('add')
    .description('Add a new generic connection profile')
    .argument('<id>', 'Profile ID')
    .argument('<kind>', 'Connector kind (sharepoint|onedrive|googledrive|s3|local|dataverse|...)')
    .option('--name <name>', 'Display name (defaults to the id)')
    .option('--field <key=value...>', 'Non-secret config field, repeatable', (val, prev: string[]) => (prev || []).concat(val), [])
    .option('--secret <secret>', 'Secret payload (client secret, access key, service-account key, ...)')
    .action(async (id, kind, options) => {
        const env = new CliEnvironment(process.cwd());
        const store = await env.loadConnectionsStore();

        const config: Record<string, string> = {};
        for (const field of options.field as string[]) {
            const eq = field.indexOf('=');
            if (eq === -1) {
                throw new Error(`Invalid --field "${field}". Expected key=value.`);
            }
            config[field.slice(0, eq)] = field.slice(eq + 1);
        }

        store[id] = { id, name: options.name || id, kind, config };
        await env.saveConnectionsStore(store);

        if (options.secret !== undefined) {
            await env.setSecret(id, options.secret);
        }

        console.log(`Connection profile ${id} (${kind}) added successfully.`);
    });

connectionsCmd.command('remove')
    .description('Remove a generic connection profile')
    .argument('<id>', 'Profile ID')
    .action(async (id) => {
        const env = new CliEnvironment(process.cwd());
        const store = await env.loadConnectionsStore();
        if (store[id]) {
            delete store[id];
            await env.saveConnectionsStore(store);
            await env.deleteSecret(id);
            console.log(`Connection profile ${id} removed.`);
        } else {
            console.log(`Connection profile ${id} not found.`);
        }
    });

connectionsCmd.command('update')
    .description('Update an existing generic connection profile')
    .argument('<id>', 'Profile ID')
    .option('--name <name>', 'Display name')
    .option('--field <key=value...>', 'Non-secret config field to set/override, repeatable', (val, prev: string[]) => (prev || []).concat(val), [])
    .option('--secret <secret>', 'Secret payload to overwrite')
    .action(async (id, options) => {
        const env = new CliEnvironment(process.cwd());
        const store = await env.loadConnectionsStore();
        if (!store[id]) {
            console.log(`Connection profile ${id} not found. Use 'add' to create it.`);
            return;
        }

        if (options.name) {
            store[id].name = options.name;
        }
        for (const field of options.field as string[]) {
            const eq = field.indexOf('=');
            if (eq === -1) {
            ...
            store[id].config[field.slice(0, eq)] = field.slice(eq + 1);
        }
        ...
        console.log(`Connection profile ${id} updated successfully.`);
    });
