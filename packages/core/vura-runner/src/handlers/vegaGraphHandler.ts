 ...
 *   - cell.metadata.graphSourceCellName  → that cell's table name, used to re-locate it if the
 *                                          stored index has gone stale (cells inserted/removed/
 *                                          reordered since the source was picked)
...
export async function handleVegaGraph(
    ...
    if (spec.data !== undefined) {
        ...
        // The stored index is a position, not a stable id — cells inserted, removed,
        // or reordered above this one since the source was picked shift it out from
        // under the reference. Prefer it when it still points at a JSON cell, but
        // fall back to re-locating the source by its (stable) table name.
        let resolvedIndex = sourceCellIndex;
        if (
            resolvedIndex < 0 ||
            resolvedIndex >= notebookCells.length ||
            notebookCells[resolvedIndex].language !== 'json'
        ) {
            resolvedIndex = sourceCellName === undefined ? -1 : notebookCells.findIndex((c, idx) =>
                c.language === 'json' && (c.metadata?.tableName || `cell_${idx}`) === sourceCellName
            );

            if (resolvedIndex === -1) {
                throw new Error(
                    sourceCellName
                        ? `Could not find the JSON Compose source cell "${sourceCellName}" (it may have been deleted or renamed). Pick a new graph source.`
                        : `Source cell index ${sourceCellIndex} is out of range.`
                );
            }
        }

        const sourceCell = notebookCells[resolvedIndex];
        ...
        if (!jsonOutputStr) {
            if (sourceCell.language === 'json') {
                ...
                // Unreachable in practice: resolvedIndex is only ever set to a cell
                // already confirmed to have language 'json' above. Kept as a guard.
                throw new Error(
                    `Source cell ${resolvedIndex + 1} has no JSON output and is not a JSON Compose cell.`
                );
            }
        }
