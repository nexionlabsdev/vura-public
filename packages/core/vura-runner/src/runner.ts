...
import { IVuraEnvironment, ICellLogger, FlownbCell, NotebookExecutionResult, NotebookExecutionOptions, CellExecutionResult } from './interfaces';
  ...
  render();
    ...
    public async executeNotebook(
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            // ECR-2: pause gate (awaited), then ECR-1: cancel check — both BEFORE the cell starts.
            if (options?.beforeCell) {
                try { await options.beforeCell(i, cells.length); } catch (e: any) {
            ...
            try {
                if (signal) {
                    // ECR-1: race the cell against the abort. The abort handler interrupts an in-flight
                    // DuckDB query; a cell in an external process cannot be interrupted from here.
                    const running = this.executeCell(cell, i, cells, logger);
                    running.catch(() => { /* its outcome no longer matters once canceled */ });
                    let onAbort: (() => void) | undefined;
                    const aborted = new Promise<never>((_, reject) => {
                        onAbort = () => { this._duckDbInstance?.interrupt(); reject(new Error('canceled')); };
                        if (signal.aborted) onAbort(); else signal.addEventListener('abort', onAbort, { once: true });
                    });
                    try { await Promise.race([running, aborted]); } finally { if (onAbort) signal.removeEventListener('abort', onAbort); }
                } else {
                ...
                const msg = signal?.aborted ? 'canceled' : (err.message || String(err));
        ...
        return {
            status: canceled ? 'canceled' : firstError ? 'error' : 'success',
            context: ctxManager.getContext(),
            httpOutputCell,
            httpOutputCellIndex,
            error: canceled ? 'canceled' : firstError
        };
    }
