...
describe('Sidecar E2E Process Spawn Tests (JS & Py)', () => {
    ...
    test.each([
        ['Python sidecar', () => spawn(pythonBin, [pythonSidecar, '--serve'], {
...
`;
    ...
    test('>1MB BLOB round-trip through spawned sidecar process asserting exact Buffer', async () => {
        ...
        const worker = await sidecarPool.acquire(poolKey, () => spawn(pythonBin, [pythonSidecar, '--serve'], {
...
`;
    ...
    test.each([
        ['Python sidecar', (pauseEnvKey: string | null) => spawn(pythonBin, [pythonSidecar, '--serve'], {
            env: {
                ...process.env,
