...
```typescript
const core = vscode.extensions.getExtension('nexion-labs.vura-studio-lite');
if (core) {
    if (!core.isActive) {
        await core.activate();
    }
