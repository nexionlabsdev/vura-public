...
// ─── Schema introspection (additive; per-connector describeSchema()) ───────

export interface NormalizedField {
    name: string;
    type: 'string' | 'number' | 'boolean' | 'datetime' | 'guid' | 'reference' | 'unknown';
    nullable: boolean;
}

/** Left out entirely (not guessed) when the source has no native way to express it. */
export interface NormalizedRelationship {
    sourceField: string;
    targetEntity: string;
    targetField: string;
    kind: 'foreign-key' | 'lookup' | 'none-detected';
}
...
export interface NormalizedEntity {
    ...
    /** True when fields/types came from a bounded content sample (e.g. S3/local
     *  schema-on-read) rather than an authoritative schema/metadata API. */
    inferred: boolean;
}
...
export interface IConnectionAdapter {
    ...
    /** Introspects the connection's source schema into a normalized shape. Optional so any
     *  existing IConnectionAdapter implementer that hasn't added it yet still type-checks —
     *  mirrors the `?` pattern already used on IVuraProvider.getConnectorKind?/getConnectionFields?/testConnection?. */
    describeSchema?(connectionId: string): Promise<NormalizedSchema>;
}
