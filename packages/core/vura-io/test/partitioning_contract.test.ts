...
describe('Phase 6 Partitioning & Threshold Contract Tests (TS)', () => {
    ...
    test('handles timestamp and epoch values correctly in partitioned tables', async () => {
        const expectedIso = new Date(1789499102000).toISOString();
        const rows = Array.from({ length: 10 }, (_, i) => ({
            id: i,
            created_at: i === 0 ? new Date(1789499102000) : 1789499102000
        }));
        await dataMgr.put('ts_part_tbl', rows);

        const manifestFile = path.join(tmpDir, 'ts_part_tbl', 'manifest.json');
        expect(fs.existsSync(manifestFile)).toBe(true);

        const readBack = await dataMgr.get('ts_part_tbl');
        expect(readBack.length).toBe(10);
        expect(readBack[0].created_at).toBe(expectedIso);
        expect(readBack[1].created_at).toBe(expectedIso);
    });
});
